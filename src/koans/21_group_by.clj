...
(meditations
  ...
  (= {"Bob" [{:name "Bob" :id 1}]
   ...
   (group-by :name [{:id 1 :name "Bob"} {:id 2 :name "Mike"} {:id 1 :last-name "Smith"}]))
  ...
  (= {:naughty-list [{:name "Jimmy", :bad true} {:name "Joe", :bad true}], :nice-list [{:name "Jack", :bad false}]}
     (group-by #(if (:bad %) :naughty-list :nice-list) [{:name "Jimmy" :bad true} {:name "Jack" :bad false} {:name "Joe" :bad true}])))
